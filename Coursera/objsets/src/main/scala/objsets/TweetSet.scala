...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def descendingByRetweet: TweetList = {
    val mostRw = mostRetweeted
    new Cons(mostRw, remove(mostRw).descendingByRetweet)
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = entityTweets(google)
  lazy val appleTweets: TweetSet = entityTweets(apple)

  def entityTweets(ent: List[String]): TweetSet = {
    TweetReader.allTweets.filter { t => ent.exists { w => t.text.contains(w) } }
  }
